...
// Override socket connection to track online status
io.on('connection', (socket) => {
  // already handled above, but track online
});

// Track online in existing connection handler — patch via Map in auth middleware
// We'll use a simple API endpoint
...
app.post('/api/posts/:postId/bookmark', authMiddleware, async (req, res) => {
  try {
    ...
    const idx = user.bookmarks.indexOf(req.params.postId);
    if (idx === -1) {
...
app.get('/api/posts/bookmarks', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user._id);
    if (!user.bookmarks || user.bookmarks.length === 0) return res.json([]);
    ...
    res.json(posts.map(p => ({
      ...p.toObject(),
      user: { ...p.user.toObject(), isFollowing: followingSet.has(p.user._id.toString()) }
    })));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
    ...
    if (message.chat) {
      ...
      if (chat) {
        chat.participants.forEach(p => {
          io.to(p.toString()).emit('message reaction', { messageId: message._id, reactions: message.reactions });
        });
      }
    } else if (message.group) {
      io.to(`group_${message.group}`).emit('message reaction', { messageId: message._id, reactions: message.reactions });
    }
...
// ========== Profile Views ==========
app.post('/api/users/:userId/view', authMiddleware, async (req, res) => {
  try {
    if (req.params.userId === req.user._id.toString()) return res.json({ ok: true });
    const user = await User.findById(req.params.userId);
    if (!user) return res.status(404).json({ error: 'User not found' });
    if (!user.profileViews) user.profileViews = [];
    const alreadyViewed = user.profileViews.find(v => v.viewer && v.viewer.toString() === req.user._id.toString());
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    if (!alreadyViewed || alreadyViewed.viewedAt < oneDayAgo) {
      user.profileViews = user.profileViews.filter(v => v.viewer && v.viewer.toString() !== req.user._id.toString());
      user.profileViews.push({ viewer: req.user._id, viewedAt: new Date() });
      await user.save();
    }
    res.json({ ok: true });
  } catch (err) {
...
app.get('/api/users/me/profile-views', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate('profileViews.viewer', 'name username profilePic verified');
