...
async function apiRequest(endpoint, options = {}) {
  ...
  // Cache GET requests for 20 seconds
  const isGet = !options.method || options.method === 'GET';
  if (isGet) {
    const cached = _apiCache.get(endpoint);
    if (cached && Date.now() - cached.t < 20000) return cached.d;
  }
  ...
  if (isGet) _apiCache.set(endpoint, { d: data, t: Date.now() });
  else _apiCache.clear(); // clear cache on mutations
  return data;
}
...
function updateOnlineStatus(userId, isOnline) {
  document.querySelectorAll(`[data-online-id="${userId}"]`).forEach(dot => {
    dot.style.display = isOnline ? 'block' : 'none';
  });
}

function isOnline(userId) {
  return onlineUserIds.has(userId ? userId.toString() : '');
}
...
function initApp() {
  ...
  socket.on('message reaction', (data) => {
    ...
    if (msgEl) {
      let rd = msgEl.querySelector('.msg-reactions');
      if (!rd) { rd = document.createElement('div'); rd.className = 'msg-reactions'; msgEl.appendChild(rd); }
      const grouped = {};
      data.reactions.forEach(r => { grouped[r.emoji] = (grouped[r.emoji] || 0) + 1; });
      rd.innerHTML = Object.entries(grouped).map(([e, c]) => `<span class="reaction-pill">${e} ${c}</span>`).join('');
    }
  });
  ...
  // Load bookmarks & online users (non-blocking)
  apiRequest('/api/posts/bookmarks').then(posts => posts.forEach(p => userBookmarks.add(p._id))).catch(() => {});
  apiRequest('/api/users/online').then(d => d.onlineIds.forEach(id => onlineUserIds.add(id))).catch(() => {});

  // Load initial badge counts
  apiRequest('/api/notifications').then(notifs => {
    const unread = notifs.filter(n => !n.read).length;
    updateNotifBadge(unread);
  }).catch(() => {});

  apiRequest('/api/chats').then(chats => {
...
function renderPost(post) {
  ...
  const bookmarked = userBookmarks.has(post._id);
  ...
  return `
    <div class="post-card" data-post-id="${post._id}">
      ...
      <div class="post-actions">
        ...
        <button class="bookmark-btn ${bookmarked ? 'bookmarked' : ''}" onclick="event.stopPropagation(); toggleBookmark('${post._id}', this)" title="Save post">
...
window.toggleBookmark = async (postId, btn) => {
  try {
    const data = await apiRequest(`/api/posts/${postId}/bookmark`, { method: 'POST' });
    if (data.bookmarked) {
      ...
      showPopup('Removed from saved', 'info');
    }
...
async function loadNotifications() {
  try {
    ...
    // Clear badge since user is viewing
    updateNotifBadge(0);
    await apiRequest('/api/notifications/read-all', { method: 'POST' }).catch(() => {});
...
async function deleteAccount() {
  ...
  if (!passwordEl) return showPopup('Error finding password field', 'error');
  const password = passwordEl.value;
  if (!password) return showPopup('Enter your password to delete account', 'warning');
...
async function openChat(otherUserId, chatId, addToHistory = true) {
  ...
  document.getElementById('chat-header-info').innerHTML = `
    <div style="position:relative;display:inline-block;">
      ...
      <span data-online-id="${otherUserId}" style="display:${online ? 'block' : 'none'};position:absolute;bottom:1px;right:1px;width:10px;height:10px;background:#22c55e;border-radius:50%;border:2px solid var(--bg-primary);"></span>
    </div>
    <div>
      <strong>${otherUser.name}</strong>
      ${otherUser.verified ? '<span class="verified-badge">✓</span>' : ''}
      ${otherUser.ownerBadge ? '<span class="owner-badge">👑</span>' : ''}
      <div style="font-size:11px;color:${online ? '#22c55e' : 'var(--text-secondary)'};">${online ? 'Online' : 'Offline'}</div>
    </div>
  `;
    ...
    return headerHtml + `
      <div class="message ${isOwn ? 'own' : ''}" data-message-id="${m._id}">
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-top:2px;">
          ...
          <button onclick="showReactionPicker('${m._id}')" style="background:none;border:none;cursor:pointer;font-size:14px;opacity:0.5;padding:0 2px;">😊</button>
          ${isOwn ? `<button class="delete-message" onclick="deleteMessage('${m._id}')" style="background:none;border:none;cursor:pointer;color:var(--text-secondary);padding:0 2px;"><i class="fas fa-trash" style="font-size:11px;"></i></button>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
function handleIncomingPrivateMessage(data) {
  // Update chat badge if not in chats view
  if (!document.getElementById('chats-view')?.classList.contains('active') || !activeChatId) {
    const badge = document.getElementById('chat-badge');
    const current = parseInt(badge?.textContent) || 0;
    updateChatBadge(current + 1);
  }
...
window.showReactionPicker = (messageId) => {
  ...
  picker.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--bg-secondary);border-radius:30px;padding:10px 16px;display:flex;gap:14px;z-index:9999;box-shadow:0 4px 24px rgba(0,0,0,0.5);';
  picker.innerHTML = emojis.map(e => `<span style="font-size:26px;cursor:pointer;" onclick="sendReaction('${messageId}','${e}')">${e}</span>`).join('');
  document.body.appendChild(picker);
  setTimeout(() => picker.remove(), 4000);
};

window.sendReaction = async (messageId, emoji) => {
  document.getElementById('reaction-picker')?.remove();
  try {
    await apiRequest(`/api/messages/${messageId}/react`, { method: 'POST', body: JSON.stringify({ emoji }) });
  } catch(e) { showPopup(e.message, 'error'); }
};

// ===== Profile Views =====
window.showProfileViews = async () => {
  try {
    const views = await apiRequest('/api/users/me/profile-views');
    const modal = document.getElementById('profile-views-modal');
    const list = document.getElementById('profile-views-list');
    if (!modal) return;
    list.innerHTML = views.length === 0
      ? '<p style="color:var(--text-secondary);text-align:center;padding:20px;">No profile views yet</p>'
      : views.map(v => `
          <div class="user-item" onclick="openProfile('${v.viewer._id}'); document.getElementById('profile-views-modal').classList.remove('active');">
            ${v.viewer.profilePic ? `<img src="${v.viewer.profilePic}" style="width:45px;height:45px;border-radius:50%;">` : `<div class="default-avatar" style="width:45px;height:45px;">${v.viewer.name.charAt(0).toUpperCase()}</div>`}
            <div class="user-info">
              <h4>${v.viewer.name} ${v.viewer.verified ? '<span class="verified-badge">✓</span>' : ''}</h4>
              <p>@${v.viewer.username} · ${formatTime(v.viewedAt)}</p>
            </div>
          </div>`).join('');
    modal.classList.add('active');
  } catch(e) { showPopup(e.message, 'error'); }
...
window.loadBookmarks = async () => {
  try {
    const posts = await apiRequest('/api/posts/bookmarks');
    const modal = document.getElementById('bookmarks-modal');
    const list = document.getElementById('bookmarks-list');
    if (!modal) return;
    list.innerHTML = posts.length === 0
      ? '<p style="color:var(--text-secondary);text-align:center;padding:20px;">No saved posts yet</p>'
