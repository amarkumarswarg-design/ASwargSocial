...
function initApp() {
  ...
  // Load bookmarks
  try {
    const bookmarkPosts = await apiRequest('/api/posts/bookmarks');
    bookmarkPosts.forEach(p => userBookmarks.add(p._id));
  } catch(e) {}

  // Load online users
  try {
    const { onlineIds } = await apiRequest('/api/users/online');
  ...
  return `
    <div class="post-card" data-post-id="${post._id}">
      ...
      <div class="post-actions">
        ...
        <button class="bookmark-btn ${bookmarked ? 'bookmarked' : ''}" data-post-id="${post._id}" onclick="event.stopPropagation(); toggleBookmark('${post._id}', this)">
...
async function toggleBookmark(postId, btn) {
  try {
    ...
    if (data.bookmarked) {
      ...
      showPopup('Post removed from saved', 'info');
    }
  } catch (err) { showPopup(err.message, 'error'); }
}
...
function handleMessageReaction(data) {
  ...
  if (msgEl) {
    ...
    reactDiv.innerHTML = Object.entries(grouped).map(([emoji, count]) => 
...
async function renderProfile(userId) {
  ...
  if (!isOwn) {
    apiRequest(`/api/users/${userId}/view`, { method: 'POST' }).catch(() => {});
  }
  try {
    ...
    let profileViews = [];
    if (isOwn) {
      try { profileViews = await apiRequest('/api/users/me/profile-views'); } catch(e) {}
    }
        ...
        <div class="profile-header glass-card">
          ...
          <div>
            ...
            <div class="profile-stats">
              ...
              ${isOwn ? `
              <div class="stat" id="views-stat" style="cursor:pointer;" onclick="showProfileViews()">
                <div class="stat-number">${profileViews.length}</div>
...
function openEditProfileModal() {
  ...
  const picEl = document.getElementById('edit-profile-pic');
  if (picEl) {
    picEl.src = currentUser.profilePic || '';
    picEl.style.display = currentUser.profilePic ? 'block' : 'none';
  }
  editDpBase64 = '';
  modal.classList.add('active');
}
...
async function deleteAccount() {
  ...
  if (!passwordEl) return showPopup('Error: password field not found', 'error');
  const password = passwordEl.value;
  if (!password) return showPopup('Enter your password to delete account', 'warning');
  showPopup('This will permanently delete your account and all data. Are you sure?', 'confirm', async (confirmed) => {
    if (confirmed) {
      try {
        showLoading();
...
async function openChat(otherUserId, chatId, addToHistory = true) {
  ...
  document.getElementById('chat-header-info').innerHTML = `
    <div style="position:relative;display:inline-block;">
      ...
      <span class="online-dot" style="display:${isOnline(otherUserId) ? 'block' : 'none'}; position:absolute;bottom:2px;right:2px;width:10px;height:10px;background:#22c55e;border-radius:50%;border:2px solid var(--bg-primary);"></span>
    </div>
    <div>
      <strong>${otherUser.name}</strong>
      ${otherUser.verified ? '<span class="verified-badge">✓</span>' : ''}
      ${otherUser.ownerBadge ? '<span class="owner-badge">👑</span>' : ''}
      <div style="font-size:11px;color:${isOnline(otherUserId) ? '#22c55e' : 'var(--text-secondary)'};">
  ...
  container.innerHTML = messages.map(m => {
    ...
    return headerHtml + `
      <div class="message ${isOwn ? 'own' : ''}" data-message-id="${m._id}">
        ...
        <div class="msg-reactions">${(m.reactions || []).reduce((acc, r) => {
          acc[r.emoji] = (acc[r.emoji] || 0) + 1; return acc;
        }, {}) && Object.entries((m.reactions || []).reduce((acc, r) => { acc[r.emoji] = (acc[r.emoji] || 0) + 1; return acc; }, {})).map(([emoji, count]) => `<span class="reaction-pill">${emoji} ${count}</span>`).join('')}</div>
        <div class="msg-footer">
          <span class="message-time">${timeStr}</span>
          ${isOwn ? `<span class="message-status">${m.readBy?.length > 1 ? '✓✓' : '✓'}</span>` : ''}
        </div>
        <div class="msg-actions">
          <button class="react-btn" onclick="showReactionPicker('${m._id}')">😊</button>
          ${isOwn ? `<button class="delete-message" onclick="deleteMessage('${m._id}')"><i class="fas fa-trash"></i></button>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
window.showReactionPicker = (messageId) => {
  ...
  picker.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:var(--bg-secondary);border-radius:30px;padding:10px 16px;display:flex;gap:12px;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,0.4);';
  picker.innerHTML = emojis.map(e => `<span style="font-size:24px;cursor:pointer;" onclick="sendReaction('${messageId}','${e}')">${e}</span>`).join('');
...
window.showProfileViews = async () => {
  try {
    ...
    list.innerHTML = views.length === 0 ? '<p style="color:var(--text-secondary)">No profile views yet</p>' :
      views.map(v => `
        <div class="user-item" onclick="openProfile('${v.viewer._id}'); document.getElementById('profile-views-modal').classList.remove('active');">
          ${v.viewer.profilePic 
            ? `<img src="${v.viewer.profilePic}" style="width:45px;height:45px;border-radius:50%;">` 
            : `<div class="default-avatar" style="width:45px;height:45px;">${v.viewer.name.charAt(0).toUpperCase()}</div>`}
          <div class="user-info">
...
window.loadBookmarks = async () => {
  try {
    ...
    list.innerHTML = posts.length === 0 ? '<p style="color:var(--text-secondary)">No saved posts yet</p>' :
